...
- Test command: `bash scripts/verify` (runs `python -m unittest discover -s tests -p 'test_*.py'` + validator scripts + Node `scripts/provider_ui/*.test.mjs`)
- Coverage command: `python -m coverage run -m unittest discover -s tests -p 'test_*.py' && python -m coverage xml -o coverage.xml && python scripts/quality/assert_coverage_100.py --report coverage.xml` (blocked by `.coverage-thresholds.json`; PR 2 of `feat/quality-rollup-v2` wires this into `scripts/verify` — see `docs/plans/2026-04-09-quality-rollup-v2-design.md` §6)
...
- Python 3.12 target (local dev may be 3.14); type hints required on public functions; avoid bare `Any`
- **pylint** is the current linter (`.pylintrc` at repo root); **ruff**, **bandit**, and **.semgrep.yml** land as per-tool configs in QRv2 PR 2
- 100% line+branch coverage on the control plane itself (via `scripts/quality/assert_coverage_100.py`, wired into `scripts/verify` in QRv2 PR 2)
