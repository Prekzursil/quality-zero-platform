...
def _normalized_branch_min_percent(raw_value: Any) -> float | None:
    """Clamp the optional branch threshold to a valid percentage."""
...
def parse_named_path(value: str) -> Tuple[str, Path]:
    """Split a `name=path` CLI value into a report name and filesystem path."""
...
def _branch_requirement_line(payload: Mapping[str, Any]) -> str:
    """Render the branch-threshold summary line."""
...
def _render_md(payload: Mapping[str, Any]) -> str:
    """Render the coverage gate report as markdown."""
    lines = [
        "# Coverage 100 Gate",
        "",
...
def _collect_coverage_inputs(
    ...
    """Parse all declared coverage inputs and collect their covered sources."""
    stats: List[CoverageStats] = []
...
def _build_payload(*args: Any, **kwargs: Any) -> Dict[str, Any]:
    """Build the JSON payload consumed by the report writer."""
    if args:
...
def main() -> int:
    """Run the coverage assertion command-line entrypoint."""
    args = _parse_args()
