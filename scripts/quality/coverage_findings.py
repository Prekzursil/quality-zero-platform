...
def _matches_required_source(source_path: str, required_source: str) -> bool:
    """Return whether a reported source path satisfies a required source."""
...
def _find_missing_required_sources(
    reported_sources: Set[str],
    required_sources: List[str],
) -> List[str]:
    """Return required source paths that are absent from the report."""
    return [
        normalized_required
...
def _is_tests_only_report(reported_sources: Set[str]) -> bool:
    """Return whether the coverage report references only test paths."""
...
def _coverage_threshold_findings(
    ...
    """Return findings for file and combined line coverage thresholds."""
    findings: List[str] = []
...
def _combined_branch_coverage(stats: List["CoverageStats"]) -> Tuple[int, int, float]:
    """Return combined branch totals, covered count, and percentage."""
...
def _branch_coverage_findings_for_stats(
    ...
    """Return findings for per-file branch coverage below the threshold."""
    findings: List[str] = []
    for item in stats:
...
def _branch_threshold_findings(
    stats: List["CoverageStats"],
    branch_min_percent: float | None,
) -> List[str]:
    """Return findings for missing or insufficient branch coverage."""
...
def _required_source_findings(
    ...
    """Return findings for missing required first-party source paths."""
    findings: List[str] = []
