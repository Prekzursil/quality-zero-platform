...
def _matches_required_source(source_path: str, required_source: str) -> bool:
    ...
    return bool(normalized_required) and (source_path == normalized_required or source_path.startswith(f"{normalized_required}/"))
...
def _is_tests_only_report(reported_sources: Set[str]) -> bool:
    ...
    return bool(reported_sources) and all(source_path == "tests" or source_path.startswith("tests/") for source_path in reported_sources)
...
def _coverage_threshold_findings(stats: List["CoverageStats"], min_percent: float) -> List[str]:
    ...
    for item in stats_list:
        if item.percent < min_percent:
            findings.append(f"{item.name} coverage below {min_percent:.2f}%: {item.percent:.2f}% ({item.covered}/{item.total})")
    ...
    if combined < min_percent:
        findings.append(f"combined coverage below {min_percent:.2f}%: {combined:.2f}% ({combined_covered}/{combined_total})")
    return findings
...
def _branch_coverage_findings_for_stats(stats: List["CoverageStats"], branch_min_percent: float) -> List[str]:
    ...
    for item in stats:
        if item.branch_percent < branch_min_percent:
            findings.append(f"{item.name} branch coverage below {branch_min_percent:.2f}%: {item.branch_percent:.2f}% ({item.branch_covered}/{item.branch_total})")
    return findings
...
def _branch_threshold_findings(stats: List["CoverageStats"], branch_min_percent: float | None) -> List[str]:
    ...
    findings = [f"{item.name} branch coverage data missing from {item.path}" for item in stats if item.branch_total <= 0]
    ...
    if combined_total > 0 and combined < branch_min_percent:
        findings.append(f"combined branch coverage below {branch_min_percent:.2f}%: {combined:.2f}% ({combined_covered}/{combined_total})")
    return findings
...
def _required_source_findings(reported_sources: Set[str], required_sources: List[str]) -> List[str]:
    ...
    findings.extend(f"missing required source path: {missing_source}" for missing_source in _find_missing_required_sources(reported_sources, required_sources))
    return findings
