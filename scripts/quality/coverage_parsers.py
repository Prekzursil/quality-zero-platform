...
def _parse_xml_totals(text: str) -> Optional[Tuple[int, int, int, int]]:
    """Parse aggregate line and branch totals from a Cobertura XML payload."""
...
def _lcov_counter_key(line: str) -> Optional[str]:
    """Map an LCOV counter line prefix to the matching counter bucket."""
...
def _iter_included_lcov_lines(path: Path) -> List[str]:
    """Yield LCOV record lines that belong to tracked repository files."""
