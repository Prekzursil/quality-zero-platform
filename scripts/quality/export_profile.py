...
def _parse_args() -> argparse.Namespace:
    """Parse CLI arguments for the resolved-profile exporter."""
...
def _coverage_input_files(coverage: Dict[str, Any]) -> str:
    """Return the comma-separated repo-relative coverage input file list."""
    return ",".join(
        str(
            PurePosixPath("repo")
            / PurePosixPath(str(item["path"]).replace("\\", "/"))
        )
...
def _json_output(key: str, value: object) -> str:
    """Format a GitHub output line containing JSON content."""
    return f"{key}={json.dumps(value, separators=(',', ':'))}"


def _profile_output_lines(profile: Dict[str, Any], event_name: str) -> List[str]:
    """Build the text output lines for a resolved control-plane profile."""
    ...
    codecov_enabled = str(
        bool(profile.get("enabled_scanners", {}).get("codecov", False))
    ).lower()
    qlty_enabled = str(
        bool(profile.get("enabled_scanners", {}).get("qlty", False))
    ).lower()
    return [
...
def _profile_json_output(profile: Dict[str, Any]) -> List[str]:
    """Build the JSON block output for a resolved control-plane profile."""
    return [
...
def _github_output_lines(profile: Dict[str, Any], event_name: str) -> List[str]:
    """Combine text and JSON output lines for GitHub Actions output."""
...
def _write_github_output(path: Path, profile: Dict[str, Any], event_name: str) -> None:
    """Append the resolved profile payload to a GitHub output file."""
