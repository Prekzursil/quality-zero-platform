"""Pure v2 migration transformers split from ``migrate_profiles_to_v2``.

These functions derive the canonical v2 shape from a parsed profile mapping
without touching disk. Keeping them in a cohesive module bounds the file-level
complexity of the CLI driver. ``migrate_profile`` is re-exported from
``migrate_profiles_to_v2`` to preserve the historical import surface.
"""
