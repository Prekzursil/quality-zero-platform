...
def _matches_required_context(actual_context: str, expected_context: str) -> bool:
    ...
    return (
        bool(current)
        and bool(expected)
        and (expected in (current, current.rsplit(" / ", 1)[-1]))
    )
...
def _validate_issue_policy_contract(profile: Dict[str, Any]) -> List[str]:
    ...
    if issue_policy.get("mode") not in {"zero", "ratchet", "audit"}:
        findings.append(
            f"{profile['slug']}: issue_policy.mode must be zero, ratchet, " f"or audit"
        )
...
def _validate_coverage_contract(profile: Dict[str, Any]) -> List[str]:
    ...
    if shell_name not in {"bash", "pwsh"}:
        findings.append(
            f"{profile['slug']}: coverage.command_shell must be bash or pwsh"
        )
    findings.extend(
        " ".join(
            [
