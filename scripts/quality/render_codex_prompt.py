...
def _parse_args() -> argparse.Namespace:
    """Parse CLI arguments for prompt rendering."""
...
def _artifact_lines(artifacts: List[object]) -> str:
    """Render artifact bullet lines for the prompt."""
...
def _required_contexts_lines(profile: dict, *, event_name: str) -> List[str]:
    """Render the required-contexts section for the prompt."""
...
def _render_prompt(*args: object, **kwargs: object) -> str:
    """Render the full Codex prompt from a finalized profile payload."""
    if len(args) != 1:
        raise TypeError(
