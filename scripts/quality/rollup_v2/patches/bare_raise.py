...
GENERATOR_VERSION = "bare_raise/1.0.0"
CATEGORY = "bare-raise"
...
def generate(
    ...
    match = _BARE_RAISE.match(target_line)
    if not match:
        return PatchDeclined(
            reason_code="ambiguous-fix",
            reason_text=f"line {finding.line} is not a bare raise",
            suggested_tier="llm-fallback",
        )

    indent = match.group(1)
    new_line = f'{indent}raise RuntimeError("unexpected error")\n'
