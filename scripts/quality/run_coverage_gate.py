...
def _run_shell(command: str, *, shell_name: str, cwd: Path) -> None:
    ...
    # shell=False prevents interpolation, and the repo-owned
    # profile command is passed via stdin instead of dynamic argv.
    if shell_name == "pwsh":
...
def _github_api_token() -> str:
    ...
    if not token:
        raise RuntimeError(
            "GITHUB_TOKEN or GH_TOKEN is required for "
            "non_regression coverage mode."
        )
    return token
...
def _load_baseline_coverage_payload(profile: Dict[str, Any]) -> Dict[str, Any]:
    """Load baseline coverage from the latest successful default-branch run."""
    ...
    if run_id is None:
        raise RuntimeError(
            "Unable to find a successful Quality Zero Platform run "
    ...
    with (
        zipfile.ZipFile(BytesIO(archive)) as handle,
        handle.open("coverage-100/coverage.json") as stream,
    ):
...
def main() -> int:
    ...
    _run_shell(
        ...
        shell_name=str(coverage.get("shell", "bash")),
        cwd=repo_dir,
    )

    mode = _coverage_mode(coverage, args.event_name)
    if mode == "evidence_only":
        note = str(coverage.get("evidence_note", "")).strip() or (
            "100/100 hard gate is enforced on protected branch pushes."
        )
