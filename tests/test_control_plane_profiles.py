    ...
    def test_special_repo_coverage_profiles_capture_multi_language_inputs(self) -> None:
        ...
        self.assertEqual(
            reframe_inputs,
            {
                ...
                # Python (cobertura) + Rust (lcov) coverage are generated by
                # the coverage command but were not declared as inputs, so the
                # apps/api/app, services/worker, and media-core sources showed
                # 0 coverage and tripped "missing required source path".
