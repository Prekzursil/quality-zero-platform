"""Test quality codacy compatibility."""
...
def _collect_future_import_issues(paths):
    """Handle collect future import issues."""
    future_annotations = []
...
def _collect_builtin_generic_offenders(paths):
    """Handle collect builtin generic offenders."""
    offenders = []
    for path in paths:
        text = path.read_text(encoding="utf-8")
...
class QualityCodacyCompatibilityTests(unittest.TestCase):
    """Quality Codacy Compatibility Tests."""
    ...
    def test_issue_collectors_flag_problematic_python_compatibility_patterns(self):
        """Cover issue collectors flag problematic python compatibility patterns."""
