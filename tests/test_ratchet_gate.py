...
class RunGitTests(unittest.TestCase):
    ...
    def test_called_process_error_raises_ratchet_error(self) -> None:
        ...
        self.assertIn("git rev-parse does-not-exist failed", str(ctx.exception))

    def test_oserror_raises_could_not_run_git(self) -> None:
        """When the binary cannot be spawned, OSError maps to RatchetError."""
        repo = _tmpdir(self)
        with (
            patch.object(rg.subprocess, "run", side_effect=OSError("boom")),
...
class IsNewCodeFindingTests(unittest.TestCase):
    ...
    def test_direct_hit_on_added_line(self) -> None:
        ...
        self.assertTrue(
            rg.is_new_code_finding({"file": "app/api.py", "line": 10}, added)
        )
    ...
    def test_added_file_present_but_line_not_added(self) -> None:
        ...
        self.assertFalse(
            rg.is_new_code_finding({"file": "app/api.py", "line": 99}, added)
        )
    ...
    def test_value_error_line_returns_false(self) -> None:
        ...
        self.assertFalse(
            rg.is_new_code_finding({"file": "app/api.py", "line": "x"}, added)
        )
    ...
    def test_type_error_line_returns_false(self) -> None:
        ...
        added: Dict[str, Set[int]] = {"app/api.py": {1}}
        self.assertFalse(
            rg.is_new_code_finding({"file": "app/api.py", "line": [1]}, added)
        )
    ...
    def test_non_positive_line_returns_false(self) -> None:
        ...
        added: Dict[str, Set[int]] = {"app/api.py": {1}}
        self.assertFalse(
            rg.is_new_code_finding({"file": "app/api.py", "line": 0}, added)
        )

    def test_empty_file_returns_false(self) -> None:
        """A finding with no file anchor is non-positional."""
        added: Dict[str, Set[int]] = {"app/api.py": {1}}
        self.assertFalse(rg.is_new_code_finding({"file": "", "line": 5}, added))
    ...
    def test_suffix_fallback_match_true(self) -> None:
        ...
        self.assertTrue(
            rg.is_new_code_finding({"file": "app/api.py", "line": 7}, added)
        )
    ...
    def test_suffix_fallback_line_miss_false(self) -> None:
        ...
        added: Dict[str, Set[int]] = {"apps/api/app/api.py": {7}}
        self.assertFalse(
            rg.is_new_code_finding({"file": "app/api.py", "line": 8}, added)
        )
...
class ReadProviderTotalsTests(unittest.TestCase):
    ...
    def test_totals_extracted_and_not_configured_skipped(self) -> None:
        ...
        canonical = {
            "provider_summaries": [
                {"provider": "Codacy", "total": 5},
                {"provider": "SonarCloud", "total": 3, "status": "ok"},
                {"provider": "Sentry", "status": "not-configured", "total": 9},
                {"provider": "", "total": 1},
            ]
        }
...
class MeasuredProvidersTests(unittest.TestCase):
    ...
    def test_measured_set_excludes_not_configured_and_blank(self) -> None:
        ...
        canonical = {
            "provider_summaries": [
                {"provider": "Codacy", "total": 1},
                {"provider": "Sentry", "status": "not-configured"},
                {"provider": "", "total": 2},
            ]
        }
        self.assertEqual(rg.measured_providers_from_canonical(canonical), {"Codacy"})
...
class ProvidersWithErrorsTests(unittest.TestCase):
    ...
    def test_exact_and_lowercase_lane_keys_mapped(self) -> None:
        ...
        self.assertEqual(
            rg.providers_with_errors(canonical), {"SonarCloud", "Codacy"}
        )
...
class ExpectedProvidersTests(unittest.TestCase):
    ...
    def test_list_required_contexts(self) -> None:
        ...
        self.assertEqual(
            rg.expected_providers_from_profile(profile), {"Dependabot", "QLTY"}
        )
    ...
    def test_active_required_contexts_merged(self) -> None:
        ...
        self.assertEqual(rg.expected_providers_from_profile(profile), {"Sentry"})
...
class RatchetBaselineTests(unittest.TestCase):
    ...
    def test_ceiling_unmeasured_entry_is_none(self) -> None:
        ...
        baseline = rg.RatchetBaseline(
            raw={"providers": {"Codacy": {"ceiling": 5, "measured": False}}}
        )
        self.assertIsNone(baseline.ceiling("Codacy"))
    ...
    def test_ceiling_non_numeric_value_is_none(self) -> None:
        ...
        baseline = rg.RatchetBaseline(
            raw={"providers": {"Codacy": {"ceiling": "five"}}}
        )
        self.assertIsNone(baseline.ceiling("Codacy"))
    ...
    def test_ceiling_numeric_value_returned(self) -> None:
        ...
        baseline = rg.RatchetBaseline(
            raw={"providers": {"Codacy": {"ceiling": 5.0}}}
        )
    ...
    def test_is_seeded_reflects_ceiling_presence(self) -> None:
        ...
        baseline = rg.RatchetBaseline(
            raw={"providers": {"Codacy": {"ceiling": 2}}}
        )
    ...
    def test_seed_equal_existing_logs_seed_action(self) -> None:
        ...
        baseline = rg.RatchetBaseline(
            raw={"providers": {"Codacy": {"ceiling": 5}}}
        )
        baseline.seed("Codacy", 5, "sha9", actor="human")
        self.assertEqual(baseline.raw["audit_log"][0]["action"], "seed")
...
class NewBaselineTests(unittest.TestCase):
    ...
    def test_scaffold_fields(self) -> None:
        ...
        self.assertEqual(baseline.raw["schema_version"], rg.RATCHET_SCHEMA_VERSION)
...
class LoadBaselineTests(unittest.TestCase):
    ...
    def test_valid_file_loaded_with_defaults_filled(self) -> None:
        ...
        path = _tmpdir(self) / "ratchet.json"
        path.write_text(
            json.dumps({"repo": "owner/repo", "providers": {"Codacy": {"ceiling": 2}}}),
            encoding="utf-8",
        )
...
class ClassifyUnmeasuredTests(unittest.TestCase):
    ...
    def test_expected_provider_held(self) -> None:
        ...
        verdict = rg._classify_unmeasured(
            provider="Codacy", baseline=baseline, expected={"Codacy"},
            errored=set(), current=None, ceiling=None, new_code=0,
        )
    ...
    def test_seeded_provider_held(self) -> None:
        ...
        baseline = rg.RatchetBaseline(
            raw={"providers": {"Codacy": {"ceiling": 1}}}
        )
        verdict = rg._classify_unmeasured(
            provider="Codacy", baseline=baseline, expected=set(),
            errored=set(), current=None, ceiling=1, new_code=0,
        )
    ...
    def test_errored_provider_held(self) -> None:
        ...
        verdict = rg._classify_unmeasured(
            provider="Codacy", baseline=baseline, expected=set(),
            errored={"Codacy"}, current=None, ceiling=None, new_code=0,
        )
        self.assertIsNotNone(verdict)
...
class ClassifyMeasuredTests(unittest.TestCase):
    ...
    def _baseline(self, ceiling: int) -> rg.RatchetBaseline:
        return rg.RatchetBaseline(
            raw={"providers": {"Codacy": {"ceiling": ceiling}}}
        )
    ...
    def test_unseeded_without_seed_flag_fails(self) -> None:
        ...
        verdict, changed = rg._classify_measured(
            provider="Codacy", baseline=baseline, current=4, ceiling=None,
            new_code=0, head_sha="s", seed_missing=False,
        )
    ...
    def test_unseeded_with_seed_flag_seeds_and_passes(self) -> None:
        ...
        verdict, changed = rg._classify_measured(
            provider="Codacy", baseline=baseline, current=4, ceiling=None,
            new_code=0, head_sha="s", seed_missing=True,
        )
    ...
    def test_new_code_fails_regardless_of_ceiling(self) -> None:
        ...
        verdict, changed = rg._classify_measured(
            provider="Codacy", baseline=baseline, current=2, ceiling=10,
            new_code=1, head_sha="s", seed_missing=False,
        )
    ...
    def test_regression_above_ceiling_fails(self) -> None:
        ...
        verdict, changed = rg._classify_measured(
            provider="Codacy", baseline=baseline, current=5, ceiling=3,
            new_code=0, head_sha="s", seed_missing=False,
        )
    ...
    def test_below_ceiling_auto_lowers(self) -> None:
        ...
        verdict, changed = rg._classify_measured(
            provider="Codacy", baseline=baseline, current=2, ceiling=5,
            new_code=0, head_sha="s", seed_missing=False,
        )
        self.assertEqual(verdict.state, "pass")
    ...
    def test_at_ceiling_passes_without_change(self) -> None:
        ...
        baseline = self._baseline(3)
        verdict, changed = rg._classify_measured(
            provider="Codacy", baseline=baseline, current=3, ceiling=3,
            new_code=0, head_sha="s", seed_missing=False,
        )
        self.assertEqual(verdict.state, "pass")
    ...
    def test_none_current_treated_as_zero(self) -> None:
        ...
        baseline = self._baseline(3)
        verdict, changed = rg._classify_measured(
            provider="Codacy", baseline=baseline, current=None, ceiling=3,
            new_code=0, head_sha="s", seed_missing=False,
        )
...
class ClassifyTests(unittest.TestCase):
    ...
    def test_universe_covers_measured_expected_seeded_and_totals(self) -> None:
        ...
        baseline = rg.RatchetBaseline(
            raw={"providers": {"SonarCloud": {"ceiling": 4}}}
        )
        result = rg.classify(
            baseline=baseline,
            totals={"Codacy": 2, "Irrelevant": 9},
            new_code={"Codacy": 0},
...
class RenderMarkdownTests(unittest.TestCase):
    ...
    def test_pass_verdict_marks_pass(self) -> None:
        ...
        result = rg.GateResult(
            verdicts=[rg.ProviderVerdict("Codacy", "pass", 2, 5, 0, lowered_to=2)],
            changed=True,
        )
...
class MainTests(unittest.TestCase):
    ...
    def test_seed_path_writes_baseline_and_passes(self) -> None:
        ...
        root = self._workspace()
        canonical = self._write_canonical(root, {
            "provider_summaries": [{"provider": "Codacy", "total": 3}],
            "findings": [],
        })
        ratchet = root / "ratchet.json"
        gh = root / "gh.txt"
        ...
        self.assertIn("Layer-1 Ratchet Gate", out_md.read_text(encoding="utf-8"))

    def test_auto_lower_path_writes_baseline(self) -> None:
        """A below-ceiling run auto-lowers and persists with ``--write``."""
        root = self._workspace()
        canonical = self._write_canonical(root, {
            "provider_summaries": [{"provider": "Codacy", "total": 1}],
            "findings": [],
        })
        ratchet = root / "ratchet.json"
        ratchet.write_text(json.dumps({
            "repo": "owner/repo",
            "providers": {"Codacy": {"ceiling": 5, "measured": True}},
        }), encoding="utf-8")
        code = rg.main(self._argv(
            canonical=canonical,
            ratchet=ratchet,
    ...
    def test_regression_returns_1(self) -> None:
        ...
        root = self._workspace()
        canonical = self._write_canonical(root, {
            "provider_summaries": [{"provider": "Codacy", "total": 9}],
            "findings": [],
        })
        ratchet = root / "ratchet.json"
        ratchet.write_text(json.dumps({
            "providers": {"Codacy": {"ceiling": 3, "measured": True}},
        }), encoding="utf-8")
        code = rg.main(self._argv(
            canonical=canonical,
            ratchet=ratchet,
    ...
    def test_new_code_returns_1(self) -> None:
        ...
        canonical = self._write_canonical(repo, {
            "provider_summaries": [{"provider": "Codacy", "total": 1}],
            "findings": [
                {
                    "file": "mod.py",
    ...
    def test_unmeasured_hold_returns_1(self) -> None:
        ...
        profile.write_text(json.dumps({
            "required_contexts": {"always": ["Codacy analysis"]},
        }), encoding="utf-8")
        ratchet = root / "ratchet.json"
        code = rg.main(self._argv(
            canonical=canonical,
    ...
    def test_unseeded_returns_1(self) -> None:
        ...
        root = self._workspace()
        canonical = self._write_canonical(root, {
            "provider_summaries": [{"provider": "Codacy", "total": 2}],
            "findings": [],
        })
        ratchet = root / "ratchet.json"
        code = rg.main(self._argv(
    ...
    def test_profile_json_path_missing_is_ignored(self) -> None:
        ...
        code = rg.main(self._argv(
            ...
            repo_dir=root,
            head_sha="sha6",
            extra=["--profile-json", str(root / "nope.json")],
        ))
        self.assertEqual(code, 0)
