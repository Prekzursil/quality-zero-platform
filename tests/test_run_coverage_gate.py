...
class RunCoverageGateTests(unittest.TestCase):
    ...
    def test_download_and_lookup_helpers_cover_success_paths(self) -> None:
        ...
        with patch.dict("os.environ", {}, clear=True), self.assertRaisesRegex(RuntimeError, "GITHUB_TOKEN or GH_TOKEN is required"):
    ...
    def test_remaining_helper_and_entrypoint_paths(self) -> None:
        ...
        with (
            ...
            patch.object(run_coverage_gate, "_download_bytes", return_value=json.dumps({"workflow_runs": []}).encode("utf-8")),
        ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(sys, "argv", [str(script_path), "--profile-json", str(profile_json), "--event-name", "push"]), self.assertRaises(SystemExit) as result:
